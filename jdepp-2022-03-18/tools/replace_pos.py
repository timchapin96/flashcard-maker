...
raw    = tempfile.TemporaryFile (mode="w", encoding=charset)
for line in open (sys.stdin.fileno (),  'r', encoding='euc-jp'):
    if line[:-1] == 'EOS': # EOS
        if error:
            stat['failed'] += 1
            error = False
...
raw.seek (0)
tagged = iter (subprocess.Popen (sys.argv[1:], encoding=charset, stdin=raw, stdout=subprocess.PIPE).communicate ()[0].splitlines ())
